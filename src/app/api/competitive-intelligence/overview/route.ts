...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({
      ok: true,
      window,
      shareOfAnswer: overview.shareOfAnswer,
      shareOfClickableSurface: overview.shareOfClickableSurface,
      visibilityVolatilityIndex: overview.visibilityVolatilityIndex,
      competitorSourceOverlap: overview.competitorSourceOverlap,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to load competitive overview' },
      { status: 500 },
    );
  }
}
